...
def pruefe_dokument_konsistenz(zeilen: list[dict]) -> list[Befund]:
    """Wer selbst getragene Kosten abrechnet, muss irgendwo eine Prämie haben.

    Gezählt wird **über alle Dokumente hinweg**, je Person. Gibt es eine
    Prämie, reicht das — egal in welchem Dokument sie steht.

    Vorher galt die Regel je Dokument: eine Leistungsabrechnung, die selbst
    getragene Kosten ausweist und keine Prämie, war „unstimmig". Das ist genau
    ihr Zweck — die Prämie steht auf der Prämienbescheinigung, einem anderen
    Dokument. Die Regel meldete damit dauerhaft einen Befund, den niemand
    schliessen konnte (260923-dua).

    Der Name der Funktion bleibt, damit die Aufrufer nicht wandern müssen; die
    Frage ist jetzt eine Personen-, keine Dokumentfrage. Sie nennt deshalb eine
    Rolle statt eines Dateinamens — und ist damit teilbar.
    """
    abdeckung = sammle_abdeckung(zeilen)
    befunde: list[Befund] = []
    for rolle, a in sorted(abdeckung.items()):
        if not a.selbstkosten:
            continue
        if a.kvg or a.vvg:
            continue
        befunde.append(Befund(
            ...
            schwere="unstimmig",
            text=(f"Selbst getragene Kosten für {rolle}, aber nirgends eine "
                  f"Prämie. Wer Kosten abrechnet, ist versichert — es fehlt "
                  f"wohl die Prämienbescheinigung."),
            betrifft=rolle,
        ))
    return befunde
