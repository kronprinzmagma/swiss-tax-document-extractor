...
# Wie _N, aber auch "2 593.18" (Leerzeichen als Tausendertrenner in BANK-P TAX_P).
...
# Früher stand im Kommentar "Betrag VOR Label" (BANK-P alt);
# in aktuellen TAX_P-Dokumenten (BANK-H, BANK-P) steht das Label ZUERST.
BANK_TAX_P_BRUTTOZINS = re.compile(
    rf"Bruttozins\s+({_N})",
    flags=re.IGNORECASE,
)

# BANK-P REP_P — Gutschrift (Bruttoertrag).
...
# BANK-R Kontoauszug — Saldo zu Ihren Gunsten
BANK_RAIFFEISEN_SALDO = re.compile(
    rf"Saldo zu Ihren Gunsten\s+({_N})",
    flags=re.IGNORECASE,
)

# BANK-R Zins- und Saldoverzeichnis — Endsaldo
BANK_RAIFFEISEN_ENDSALDO = re.compile(
    rf"Endsaldo\s+({_N})",
    flags=re.IGNORECASE,
)

# BANK-R Zins- und Saldoverzeichnis / Kontoauszug — Zinszahlung (Bruttozins)
BANK_RAIFFEISEN_ZINSZAHLUNG = re.compile(
    rf"Zinszahlung\s+({_N})",
    flags=re.IGNORECASE,
)

# BANK-R Kontoauszug — Abschlussbetreffnis (Bruttozins-Buchung)
BANK_RAIFFEISEN_ABSCHLUSS = re.compile(
    rf"Abschlussbetreffnis\s+(?:von\s+\S+\s+bis\s+\S+\s+)?({_N})",
    flags=re.IGNORECASE,
)

# BANK-H / BANK-N — Guthabensaldo am [Datum] .XX [Betrag]
...
# BANK-H — alle Guthabensalden (für Multi-Konto-Dokumente)
BANK_HBL_GUTHABENSALDO_ALL = re.compile(
    rf"Guthabensaldo am\s+[\d.]+\s+(?:\.\d{{2}}\s+)?({_N})",
    flags=re.IGNORECASE,
)

# BANK-P E-Trading / Zinsabrechnung — Neuer Saldo
BANK_POSTFINANCE_NEUER_SALDO = re.compile(
    rf"Neuer Saldo\s+({_N})",
    flags=re.IGNORECASE,
)

# BANK-P TAX_P — Kontostand nach Zinsabschluss
...
# BANK-P Zinsabrechnung — Kontostand [Betrag] (Ende Dokument, ohne Suffix)
...
# KK-C kombiniert — Prämie Total CHF [Betrag] (wenn keine KVG/VVG-Trennung)
...
# KK-H — Total selbstgetragene Krankheits- und Unfallkosten [Betrag]
KK_HELSANA_EIGENANTEIL = re.compile(
    rf"Total selbstgetragene Krankheits-\s*und Unfallkosten\s+({_N})",
    flags=re.IGNORECASE,
)

# KK-V — Total von KK-V nicht getragenen Krankheitskosten in CHF [Betrag]
KK_VISANA_TOTAL = re.compile(
    rf"Total von KK-V nicht getragenen Krankheitskosten in CHF\s+({_N})",
    flags=re.IGNORECASE,
)
...
# BROKER-S / BROKER-X — Total Steuerwert [Betrag]
WERTSCHRIFTEN_SELMA_STEUERWERT = re.compile(
    rf"Total Steuerwert\s+({_N})",
    flags=re.IGNORECASE,
)

# BROKER-T — [Betrag](1) (1) Davon A ... (Fussnoten-Format)
# BROKER-T verwendet Komma als Tausendertrennzeichen (z.B. "7,793")
_N_WITH_COMMA = r"[\d,'][\d'.,]*"
...
def regex_overrides(plain_text: str, belegtyp: str) -> dict[str, str]:
    ...
    if belegtyp == "lohnausweis":
        ...
        # --- Bruttozins ---
        # Prio 1: "Bruttozins [Betrag]" (TAX_P BANK-H/BANK-P neu, label-first)
        ...
        # Prio 3: BANK-R "Zinszahlung"
        if "bruttoertrag" not in overrides:
            v = _first(BANK_RAIFFEISEN_ZINSZAHLUNG, plain_text)
            if v:
                overrides["bruttoertrag"] = v

        # Prio 4: BANK-R "Abschlussbetreffnis"
        if "bruttoertrag" not in overrides:
        ...
        # 6) BANK-P Zinsabrechnung / REP_P: letzter "Kontostand [Betrag]"
        ...
        # KVG-Prämie: KK-C/KK-H "Grundversicherung (KVG)"
        v = _first(KK_CSS_KVG, plain_text)
        if v:
            overrides["praemie_kvg_total"] = v

        # VVG-Prämie: KK-C "Zusatzversicherung (VVG)" oder KK-S-Format
        ...
        # Selbstgetragene Kosten: KK-H Jahresübersicht
        v = _first(KK_HELSANA_EIGENANTEIL, plain_text)
        if v:
            overrides["betrag"] = v
        # KK-V Jahresübersicht
        ...
        # Bruttoertrag: "Total Ertrag" nach "Subtotal Ertrag" (BROKER-T / BROKER-S)
