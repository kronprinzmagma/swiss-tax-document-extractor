...
class BankZinsausweisRaw(BaseModel):
    ...
    * ``institut`` — Name der Bank (z.B. „BANK-Z").
    ...
    * ``kontotyp`` — z.B. „Sparkonto", „Privatkonto"; deklarativ und nicht
    ...
    institut: TaggedField[str] = Field(
        ...,
        description="Name der Bank (z.B. 'BANK-Z', 'BANK-U', 'BANK-R').",
    )
...
class KkPraemienbescheinigungRaw(BaseModel):
    ...
    * ``kasse`` — Krankenversicherer (z.B. „KK-H", „KK-S", „KK-C").
...
class Saeule3aRaw(BaseModel):
    ...
    * ``stiftung`` — 3a-Stiftung (z.B. „STIFTUNG-V", „STIFTUNG-F", „BANK-P
      Vorsorge 3a").
...
class WertschriftenverzeichnisRaw(BaseModel):
    ...
    * ``waehrung`` — Default CHF.
    ...
    institut: TaggedField[str] = Field(
        ...,
        description="Name Bank/Broker (z.B. 'BANK-U AG', 'BANK-Z', 'STIFTUNG-V'). tag_refs MUSS ALLE Wörter des Namens umfassen.",
    )
    kontoinhaber_name: TaggedField[str] = Field(
        ...,
...
class HypothekZinsbestaetigungRaw(BaseModel):
    ...
    * ``institut`` — Hypothek-gebende Bank (z.B. BANK-U, BANK-Z, BANK-M).
    * ``kontoinhaber_name`` — für Person-Inferenz (D-D1).
    ...
    * ``jahr`` — 4-stellig.
    ...
    institut: TaggedField[str] = Field(
        ...,
        description="Name der Hypothek-gebenden Bank (z.B. 'BANK-U', 'BANK-Z', 'BANK-R', 'BANK-M'). tag_refs MUSS ALLE Wörter des Namens umfassen.",
    )
    kontoinhaber_name: TaggedField[str] = Field(
        ...,
