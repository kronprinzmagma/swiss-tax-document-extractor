...
def rule_saeule_3a_cap(values: dict[str, str]) -> list[str]:
    """0 < Einzahlung ≤ gesetzlicher Höchstabzug.

    Massgeblich ist der Höchstabzug MIT Pensionskasse. Der höhere Betrag
    gilt nur ohne 2. Säule, in der Praxis also für Selbständige — in diesem
    Haushalt sind beide Erwachsenen angestellt.

    Die Zahl stand hier fest verdrahtet auf dem Stand 2024 (35'280) und wäre
    für das Steuerjahr 2025 falsch gewesen — eine Einzahlung von 36'000 hätte
    einen Befund ausgelöst, obwohl sie zulässig ist. Sie kommt jetzt aus
    ``extractors.schlusspruefung``, wo sie mit Quelle steht (260923-dua).
    """
    from extractors.schlusspruefung import SAEULE_3A_MAX
    ...
    if betrag > SAEULE_3A_MAX:
        return [
            f"Säule 3a: Einzahlung {betrag:.2f} > Cap "
            f"{SAEULE_3A_MAX:,.0f} — über Maximum".replace(",", "'")
        ]
    return []
