...
Steuerjahr-Heuristik: PostFinance-Belege werden Anfang Folgejahr generiert
...
# PostFinance-Belege: Dateiname enthält Generierungsdatum als YYYYMMDD.
...
def _plausible_tax_year(years: list[int]) -> int | None:
    ...
    Wenn ein Beleg "20221231" und "20230101" enthält (PostFinance-Pattern),
...
def infer_jahr_from_filename(pdf_name: str) -> tuple[str, str] | tuple[None, None]:
    ...
    * ``Steuernachweis 2022 Muster.pdf`` → ``("2022", "filename_rule:year_in_name")``
    ...
    # 0. PostFinance early-year Spezialregel (TAX_P / REP_P / Portfolio etc.).
