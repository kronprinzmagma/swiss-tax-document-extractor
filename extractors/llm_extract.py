...
- arbeitgeber ist der FIRMENNAME (z.B. "Universität Zürich", "Post CH AG"), nicht eine Adresse.
...
- jahr: 4-stelliges Steuerjahr. Bei PostFinance/Raiffeisen-Dokumenten typisch 2022 oder 2023.

Dokumenttypen und Schlüssellabels:
- PostFinance TAX_P / Zinsabschluss: Wegen der Spaltenreihenfolge im PDF steht der Betrag VOR dem Label im Textstream. Suche das Wort "Bruttozins" und nimm die LETZTE Zahl die unmittelbar davor steht (ohne andere Wörter dazwischen). Alle Ziffern/Zahlen NACH dem Wort "Bruttozins" sind NICHT der Bruttozins-Wert. Ignoriere "Nettozins" vollständig. Der letzte Kontostand → vermoegensstand_3112.
- Raiffeisen Kontoauszug: "Saldo zu Ihren Gunsten" → vermoegensstand_3112; "Abschlussbetreffnis" → bruttoertrag
- Zins- und Saldoverzeichnis Raiffeisen: "Endsaldo" → vermoegensstand_3112; "Zinszahlung" → bruttoertrag
- PostFinance ZINSABSCHLUSS (REP_P): "Kontostand" nach "Total" → vermoegensstand_3112
- HBL/Neon Kapitalbescheinigung: "Guthabensaldo" → vermoegensstand_3112
...
- kasse: Name der Krankenkasse (z.B. "CSS", "Sanitas", "Helsana", "Swica").
- jahr: 4-stelliges Steuerjahr.

Labels nach Kasse:
- CSS / Helsana: "Grundversicherung (KVG)  [Betrag]" → praemie_kvg_total
- CSS / Helsana: "Zusatzversicherung (VVG)  [Betrag]" → praemie_vvg_total
- Sanitas: "VVG (Januar–Dezember)  Total  [Betrag]" → praemie_vvg_total (KVG oft nicht im Dokument)
...
- stiftung: Name der Vorsorgestiftung / des Anbieters (z.B. "VZ Vorsorgestiftung 3a", "VIAC", "Frankly").
...
- PostFinance Vorsorge 3a / VIAC / Frankly: abweichendes Format — "Einzahlung [Datum] CHF [Betrag]".
...
- bruttoertrag_total: Summe aller Erträge im Jahr — suche "Total Ertrag", "Bruttoertrag total", "Subtotal Ertrag". Bei Selma Finance/Saxo: Summe von Ertrag A + Ertrag B.
...
- institut: Bank / Broker / Vermögensverwalter (z.B. "Selma Finance AG", "PostFinance AG", "Raiffeisen").
- jahr: 4-stelliges Steuerjahr.

Dokumenttypen:
- Selma Finance Steuerausweis (Custodian: Saxo Bank): "Total Steuerwert [Betrag]" → bestand_3112; "Total Ertrag [Betrag]" → bruttoertrag_total; "Vermögensverwalter Selma Finance AG" → institut.
- PostFinance Portfolio-Wertentwicklung: "GESAMTVERMÖGEN IN CHF [Betrag]" (GROSSBUCHSTABEN!) → bestand_3112. Kein bruttoertrag_total (Performance-Dokument, kein Steuerausweis).
- True Wealth AG Steuerauszug: "[Betrag](1) (1) Davon A [A] und B [B]" → bestand_3112 ist [Betrag]; "Bruttoertrag ... A [A]" + "Bruttoertrag ... B [B]" → bruttoertrag_total = A + B summieren.
- PostFinance Corporate-Action (Dividende): "Betrag CHF [Brutto]" → bruttoertrag_total; "Verrechnungssteuer 35% (CH) CHF [Betrag]" → verrechnungssteuer_total.
- Raiffeisen Freies Vermögen: "Total Steuerwert" → bestand_3112.
...
- institut: Bank (z.B. "UBS", "ZKB Zürcher Kantonalbank", "Raiffeisen Schweiz").
...
B) Ein KK-"Auszug für Ihre Steuererklärung" (Helsana, CSS, Visana etc.) — jährliche Kostenübersicht
...
Typ B (KK-Jahresübersicht / "Auszug für Steuererklärung"):
  ...
  - leistungserbringer: Name der Krankenversicherung (z.B. "Helsana", "CSS", "Visana", "Swica").
...
- "Total CHF  1'357.00" (bei Jahresübersicht Visana/Helsana) → betrag value = "1'357.00"
