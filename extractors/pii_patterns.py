...
# Prüft einen kurzen, vom Menschen getippten Text auf offensichtliche PII.
#
# Gebraucht für Freitextfelder, die in der Übertragungstabelle landen — die
# Unterscheidung mehrerer Positionen eines Belegs zum Beispiel
# ("Festhypothek 0.95 %"). Dort hat eine Policen- oder Kontonummer nichts
# verloren, und die Tabelle geht an den Steuerberater.
#
# Bewusst nur die eindeutigen Muster: ein Name lässt sich in vier Wörtern
# nicht zuverlässig erkennen, und ein falscher Alarm, der die Eingabe
# wegwirft, wäre schlimmer als ein durchgelassenes Wort. Die Länge ist
# ohnehin begrenzt.
_FREITEXT_PII = (
    IBAN_REAL_RE, AHV_REAL_RE, AHV_GLUED_RE, PHONE_RE, EMAIL_RE,
    ACCOUNT_NR_DASH_RE, ACCOUNT_NR_DOT_RE,
)


def enthaelt_pii(text: str) -> bool:
    """Steht in diesem kurzen Freitext etwas, das nicht weitergegeben gehört?"""
    s = str(text or "")
    return any(muster.search(s) for muster in _FREITEXT_PII)
