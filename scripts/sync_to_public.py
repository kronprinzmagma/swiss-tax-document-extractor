...
4 Datenleck gefunden, 5 Push fehlgeschlagen.
"""
...
def copy_file(src: Path, dst: Path, rel: str, dry_run: bool = False) -> tuple[str, str | None]:
    ...
    text = raw if rel == SELF_REL else apply_substitutions(raw)
    if not dry_run:
        dst.parent.mkdir(parents=True, exist_ok=True)
