...
def _vertrag_gewinnt() -> None:
    ...
    for belegtyp, eintraege in TRANSFER_MAP.items():
        ...
        TRANSFER_MAP[belegtyp] = [
            (feld, vertrag.get(feld, label), extra)
...
#    USD-/EUR-Währungsblöcke einer PostFinance-Zinsabrechnung. Eine
...
# 3. Dieselbe Kontoposition tauchte doppelt auf, weil PostFinance für ein Konto
...
# PostFinance benennt REP_P/TAX_P nach derselben IBAN — daran hängt die
# Doppelerkennung.
...
def wende_korrekturen_an(rows: list[dict], samples_dir: Path,
    ...
    for i, r in enumerate(rows):
        schluessel = f"{r.get('pdf_name', '')}|{r.get('beschreibung', '')}"
        ...
        if eintrag is None:
            ...
            for alt_bez in ALTE_BEZEICHNUNGEN.get(r.get("beschreibung", ""), ()):
                eintrag = korrekturen.get(f"{r.get('pdf_name', '')}|{alt_bez}")
...
def ergaenze_eigene_zeilen(rows: list[dict], samples_dir: Path) -> int:
    ...
    vorhanden = {f"{r.get('pdf_name', '')}|{r.get('beschreibung', '')}"
                 for r in rows}
    ergaenzt = 0
    for schluessel, eintrag in sorted(korrekturen.items()):
        ...
        if not soll or not eintrag.get("neu"):
            continue
        beleg, _, beschreibung = schluessel.partition("|")
        rows.append({
            "steuerbereich": eintrag.get("steuerbereich", "—"),
            "belegtyp": eintrag.get("belegtyp", ""),
            "ziffer": eintrag.get("ziffer", "—"),
            "person": eintrag.get("person") or "",
...
def render_uebertragung(
    ...
    lines: list[str] = []
    lines.append(f"# Steuer-Übertragungstabelle — ZHprivateTax")
    lines.append("")
    lines.append(
        f"Quelle: `{samples_dir.relative_to(ROOT) if samples_dir.is_relative_to(ROOT) else samples_dir}/_results_full.json` "
        f"({total_belege} Belege, {extracted} extrahiert, {len(oos_entries)} out-of-scope)."
    )
    lines.append("")
    lines.append(
        "Diese Tabelle ist für die direkte Übertragung in ZHprivateTax oder den Versand "
        "an den Steuerberater bestimmt. Quelle + Anker pro Zeile siehe `nachweis_anhang.md`."
    )
    lines.append("")
    lines.append("**Spalten:** Steuerbereich · ZHprivateTax-Ziffer · Person/Rolle · "
                 "Aussteller · Beschreibung · Betrag CHF · Jahr/Periode · Quelle · Status")
    lines.append("")
    lines.append("**⧉** markiert Positionen, für die zwei Dokumente desselben Kontos "
                 "vorliegen (z.B. Zinsabrechnung *und* Steuerauszug). Beide Zeilen "
                 "bleiben stehen — übertragen wird der Betrag **einmal**. "
                 "Zeilen mit Betrag 0.00 werden nicht aufgeführt.")
    lines.append("")

    def render_table(rows: list[dict]) -> list[str]:
        out: list[str] = []
        out.append("| Steuerbereich | Ziffer | Person | Aussteller | Beschreibung | "
                   "Betrag CHF | Jahr/Periode | Quelle | Status |")
        out.append("|---|---|---|---|---|---:|---|---|---|")
        for r in rows:
            betrag = fmt_amount(r["betrag"])
            quelle = fmt_quelle(r["pdf_name"], r["page"], r.get("inference_source"))
            status_label = r["status"]
            status_md = "🟢 auto" if status_label == "auto" else "🟡 manuell prüfen"
            konf = konfidenz_fuer(r)
            status_md = f"{_KONFIDENZ_SYMBOL[konf]} {konf}"
            # Doppelte Kontoposition sichtbar machen, ohne die Zeile zu tilgen.
            if r.get("dupe_hinweis"):
                status_md += " ⧉"
            person_disp = fmt_display_value(r["person"])
            aussteller_disp = fmt_display_value(r["aussteller"])
            jahr_disp = fmt_display_value(r["jahr"])
            out.append(
                f"| {md_escape(r['steuerbereich'])} "
                f"| {md_escape(r['ziffer'])} "
                f"| {md_escape(truncate(person_disp or '', 80))} "
                f"| {md_escape(truncate(aussteller_disp or '', 80))} "
                f"| {md_escape(r['beschreibung'])} "
                f"| {betrag} "
                f"| {md_escape(jahr_disp)} "
                f"| {quelle} "
                f"| {status_md} |"
            )
        return out

    lines.append(f"## 🟢 Automatisch übertragbar ({len(auto_rows)} Positionen)")
    lines.append("")
    if auto_rows:
        # Sortierung: Steuerbereich → Ziffer → Person
        auto_sorted = sorted(
            auto_rows,
            key=lambda r: (r["steuerbereich"], r["ziffer"], r["person"] or "", r["beschreibung"]),
        )
        lines.extend(render_table(auto_sorted))
    else:
        lines.append("_Keine Positionen mit vollständigem Anker und plausiblem Wert._")
    lines.append("")

    lines.append(f"## 🟡 Manuell prüfen ({len(manual_rows)} Positionen)")
    lines.append("")
    lines.append(
        "Wert vorhanden, aber: Anker fehlt, Placeholder, ``manual_review:*``-Marker "
        "oder Plausibility-Fail im Beleg. Vor Übertragung manuell verifizieren."
    )
    lines.append("")
    if manual_rows:
        ...
        lines.extend(render_table(manual_sorted))
    else:
        lines.append("_Keine offenen Positionen._")
    lines.append("")

    lines.append(f"## ⚪ Out of Scope ({len(oos_entries)} Belege)")
    lines.append("")
    if oos_entries:
        lines.append(
            "Belege, die das Tool versteht, für diese Familie aber **keine "
            "Steuer-Übertragung benötigen** (z.B. Bestandes-Reports, Korrespondenz)."
        )
        lines.append("")
        ...
        lines.append("_Alle Belege werden für die Steuer ausgewertet._")
    lines.append("")
    return "\n".join(lines) + "\n"
