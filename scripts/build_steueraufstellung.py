...
def wende_korrekturen_an_entries(entries: list[dict],
                                 samples_dir: Path) -> tuple[int, list[dict]]:
    """Die Durchsicht des Menschen auch in die xlsx bringen.

    Die Markdown-Uebertragungstabelle laeuft durch ``wende_korrekturen_an``;
    dieser Generator tat es nicht. Die Datei, die zum Steuerberater geht,
    enthielt damit die unkorrigierten Modellwerte — und niemand haette es
    gemerkt, weil die Markdown-Tabelle daneben richtig war (260923-dua).

    Die Bruecke laeuft ueber ``build_rows``: dort traegt jede Zeile den
    ``field_name``, unter dem ihr Wert im Beleg steht. Der korrigierte Betrag
    wandert von der Zeile zurueck in genau dieses Feld.

    Returns:
        ``(angewandt, ergaenzt)`` — die Zahl geaenderter Felder und die selbst
        erfassten Positionen, fuer die es kein Feld im Beleg gibt.
    """
    from scripts.build_tax_output import (
    ...
    for e in entries:
        ...
        if not rows:
            continue
        # ``entfernen=False``: gestrichene Zeilen bleiben erkennbar, statt
        # spurlos zu verschwinden — sie werden unten als solche markiert.
        wende_korrekturen_an(rows, samples_dir, entfernen=False)
        je_feld: dict[str, dict] = {}
        for r in rows:
            name = r.get("field_name")
            # Nur die erste Position je Feld kann in ein Feld zurueck; weitere
            # haben im Beleg kein eigenes Feld und laufen ueber `ergaenzt`.
            if name and (r.get("pos") or 1) == 1 and name not in je_feld:
                je_feld[name] = r
        for f in e.get("fields") or []:
            r = je_feld.get(f.get("feld"))
            if not r:
                continue
            if r.get("gestrichen"):
                f["value"] = None
                f["korrektur"] = "gestrichen"
                angewandt += 1
                continue
            if r.get("herkunft") == "mensch" and \
                    str(f.get("value") or "") != str(r.get("betrag") or ""):
                f["value"] = r.get("betrag")
                f["korrektur"] = "mensch"
                angewandt += 1

    # Selbst erfasste Positionen: sie haben kein Feld im Beleg. Sie duerfen
    # nicht einfach fehlen — sonst ist die Aufstellung unvollstaendig, ohne es
    # zu sagen.
    ergaenzt: list[dict] = []
    leer: list[dict] = []
    ergaenze_eigene_zeilen(leer, samples_dir)
    ergaenzt.extend(leer)
    return angewandt, ergaenzt


def _build_ergaenzt_sheet(wb: openpyxl.Workbook, zeilen: list[dict]) -> None:
    """Was der Mensch selbst erfasst hat — z.B. die zweite und dritte Hypothek.

    Diese Positionen stehen in keinem Feld eines Belegs. Ohne eigenes Blatt
    fehlten sie in der Aufstellung, waehrend die Markdown-Tabelle sie fuehrt.
    """
    if not zeilen:
        return
    ws = wb.create_sheet(title="Von Hand ergänzt")
    kopf = ["Dokument", "Ziffer", "Person", "Aussteller", "Beschreibung",
            "Position", "Betrag"]
    for i, t in enumerate(kopf, start=1):
        ws.cell(row=1, column=i, value=t).font = Font(bold=True)
    for z, r in enumerate(zeilen, start=2):
        betrag = parse_swiss_amount(str(r.get("betrag") or ""))
        ws.cell(row=z, column=1, value=r.get("pdf_name", ""))
        ws.cell(row=z, column=2, value=str(r.get("ziffer") or ""))
        ws.cell(row=z, column=3, value=str(r.get("person") or ""))
        ws.cell(row=z, column=4, value=str(r.get("aussteller") or ""))
        ws.cell(row=z, column=5, value=str(r.get("beschreibung") or ""))
        ws.cell(row=z, column=6, value=int(r.get("pos") or 1))
        ws.cell(row=z, column=7, value=betrag)


def build_workbook(entries: list[dict],
                   ergaenzt: list[dict] | None = None) -> openpyxl.Workbook:
    ...
    _build_ergaenzt_sheet(wb, ergaenzt or [])
...
def main() -> int:
    ...
    angewandt, ergaenzt = wende_korrekturen_an_entries(data, samples_dir)
    wb = build_workbook(data, ergaenzt)
    wb.save(output)

    print(f"Geschrieben: {output}")
    if angewandt or ergaenzt:
        print(f"  ✍ {angewandt} korrigierte(r) Wert(e), "
              f"{len(ergaenzt)} selbst erfasste Position(en)")
