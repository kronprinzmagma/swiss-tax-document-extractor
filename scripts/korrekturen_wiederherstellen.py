...
def _zusammen(ziel: dict[str, dict], quelle: dict[str, dict]) -> tuple[int, int]:
    ...
    for schluessel, eintrag in quelle.items():
        ...
        for feld, wert in eintrag.items():
            # Ein leerer Wert ueberschreibt nie einen vorhandenen.
...
def main() -> int:
    ...
    verlauf = sorted((latest / "korrekturen-verlauf").glob("*.json"))
    verlauf += sorted((args.samples / "korrekturen-verlauf").glob("*.json"))
    for p in verlauf:
