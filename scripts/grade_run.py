...
# Klartext-Firmennamen wie "BANK-R", "BANK-P AG", "KK-S" sind
# damit verboten.
...
def check_bestaetigte_werte(samples: Path) -> CheckResult:
    ...
    try:
        from scripts.build_tax_output import (
            build_rows, ergaenze_eigene_zeilen, nummeriere_positionen,
        )
        zeilen: list[dict] = []
        for eintrag in json.loads(results.read_text()):
            if eintrag.get("status") == "ok":
                zeilen.extend(build_rows(eintrag))
        nummeriere_positionen(zeilen)
        # Selbst erfasste Positionen gehoeren dazu. Ohne sie meldete der Check
        # jede von Hand nachgetragene Zeile dauerhaft als "fehlt" — und der
        # Grader wurde nie wieder gruen, gerade weil der Mensch gearbeitet hat
        # (260923-dua).
