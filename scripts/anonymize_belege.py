...
Behaelt:
- Firmen-/Institutionsnamen (ZKB, CSS, Helsana, ...) -- noetig zur Erkennung
...
FIRM_NAMES_BY_TYPE: dict[str, list[str]] = {
    "KK": [
        "Helsana", "Sanitas", "CSS", "Visana", "KPT", "Swica", "Atupri",
        "Concordia", "Groupe Mutuel", "Mutuel", "Sympany", "ÖKK", "OEKK",
        "Assura", "Aquilana", "Vivao", "Compact", "EGK", "Innova",
        "Sanagate", "Agrisano",
    ],
    "BANK": [
        "PostFinance", "UBS", "ZKB", "Zürcher Kantonalbank",
        "Credit Suisse", "Raiffeisen", "Hypothekarbank Lenzburg", "HBL",
        "Migros Bank", "Bank Cler", "Cler", "BCV",
        "Banque Cantonale Vaudoise", "Berner Kantonalbank", "BCBE", "BLKB",
        "ZGKB", "AKB", "SGKB", "TKB", "GLKB", "OKB", "URKB",
    ],
    "BROKER": [
        "Saxo", "Saxo Bank", "Swissquote", "True Wealth", "Selma",
        "Selma Finance", "Inyova", "Yuh", "Findependent",
    ],
    "STIFTUNG_3A": [
        "VIAC", "Frankly", "finpension", "Finpension",
        "VZ Vorsorgestiftung", "VZ", "Liberty", "Tellco",
        "Sparen 3", "myThird",
    ],
}
...
RECHTSFORM_RE = re.compile(
    ...
    r"Bank|Raiffeisen|Sparkasse|Post|Finance|Broker|Management\s+AG)\b"
)
...
WICHTIG:
- KEINE Firmennamen / Institutionen ("ZKB", "Helsana", "PostFinance", "CSS", "Sanitas", "Universitaet Zuerich").
...
def build_firm_token_map(
    ...
    # denselben Marker — verhindert dass "Selma" und "Selma Finance" zwei
    ...
    # Sowohl längere ("Raiffeisenbank") als auch kürzere ("Neon") Varianten
    # MÜSSEN in die Map — die per-Token-Replacement-Pattern matchen nur
    # exakte Token. "Neon" als Standalone-Token wird NICHT vom
    # "Neon Switzerland AG"-Pattern erkannt.
    ...
    if secret_institutions:
        for inst in sorted(set(secret_institutions), key=lambda x: -len(x)):
            if not inst or inst in mapping:
                continue
            alias_marker = None
            # Kürzerer existing ⊆ inst (z.B. "Raiffeisen" ⊆ "Raiffeisenbank")
            # ODER inst ⊆ längerer existing (z.B. "Neon" ⊆ "Neon Switzerland AG")
...
def anonymize_word_text(
    ...
    # Firmen-Replace MUSS vor CamelCase-Split laufen, sonst zerlegt "PostFinance"
    # → "Post Finance" und der Map-Lookup auf "PostFinance" matched nichts mehr.
    if firm_token_map:
        for original in sorted(firm_token_map.keys(), key=lambda x: -len(x)):
            ...
            # ``CSSKranken-Versicherung`` (CSS gefolgt von uppercase K).
...
def _final_firm_sweep(pages_out: list[dict], firm_token_map: dict[str, str]) -> int:
    ...
    scheiterte (z.B. "Raiffeisenbank" oder "my CSS").
...
def anonymize_one(
    ...
    # firm_token_map. Beispiel: "True Wealth AG" → "INST-A", verteilt auf
