...
def _stationen(input_dir: Path, samples: Path) -> dict[str, dict]:
    ...
    for pdf in sorted(input_dir.glob("*.pdf")):
        stand[pdf.name] = {"im_ordner": True, "json": False, "ergebnis": None,
                           "zeilen": 0, "belegtyp": None, "aussortiert": False}
    # Abgehakte Dokumente liegen einen Ordner tiefer. Sie sind nicht
    # verschwunden, sondern ausdruecklich beiseitegelegt — als "Ergebnis ohne
    # PDF" zu melden waere ein Problem, das keines ist (260923-dua).
    for pdf in sorted((input_dir / "irrelevant").glob("*.pdf")):
        stand.setdefault(pdf.name, {"json": False, "ergebnis": None,
                                    "zeilen": 0, "belegtyp": None})
        stand[pdf.name]["im_ordner"] = True
        stand[pdf.name]["aussortiert"] = True
    ...
    if ergebnisse.exists():
        ...
        # korrekturen.json als Sample verarbeitet; der Eintrag blieb im
        # Ergebnis stehen und erschien seither als Dokument „ohne Fund", das
        # es nie gab. Die Extraktion filtert das inzwischen — das Ergebnis von
        # damals liegt aber noch da (260923-dua).
        daten = [e for e in daten
        ...
        # So zaehlen, wie es in der Tabelle steht — mit der Durchsicht.
        #
        # Vorher zaehlte `build_rows` allein. Ein Dokument, dessen Wert der
        # Mensch von Hand nachgetragen hatte, galt damit weiter als "ohne
        # Wert", und ein gestrichenes als versorgt (260923-dua).
        try:
            alle: list[dict] = []
            for e in daten:
                if e.get("status") == "ok":
                    alle.extend(build_rows(e))
            wende_korrekturen_an(alle, samples)
            ergaenze_eigene_zeilen(alle, samples)
            behalten, _ = drop_zero_rows(alle)
            for r in behalten:
                name = r.get("pdf_name") or "?"
                if name in stand:
                    stand[name]["zeilen"] = stand[name].get("zeilen", 0) + 1
        except Exception:
            for e in daten:
                if e.get("status") == "ok":
                    try:
                        stand[e.get("pdf_name") or "?"]["zeilen"] = \
                            len(build_rows(e))
                    except Exception:
                        pass
    return stand
...
def _einordnen(stand: dict[str, dict]) -> dict[str, list[str]]:
    ...
    for name, s in sorted(stand.items()):
        if s.get("aussortiert"):
            gruppen["aussortiert"].append(name)
            continue
