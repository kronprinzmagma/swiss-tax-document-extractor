...
def test_kopf_hat_die_erwarteten_spalten():
    js = _js(baue_html([], ["e1"]))
    assert kopfbreite(js) == 25


def test_kopf_kennt_die_positionsnummer():
    """Ohne sie teilen sich drei Hypotheken eines Belegs einen Schluessel und
    ueberschreiben einander (260923-dua)."""
    js = _js(baue_html([], ["e1"]))
    kopf = _array_nach(js, js.index("const kopf = ["))
    assert "'position'" in kopf
    assert "'unterscheidung'" in kopf
    # Nicht mit der angeklickten Stelle im Seitenbild verwechseln.
    assert "'pos_x'" in kopf and "'pos_seite'" in kopf
...
def test_ruecknahme_geht_auch_den_csv_weg():
    """Ohne sie war der Loeschknopf im Datei-Weg wirkungslos, und der Export
    brach mit "Nichts geändert" ab, obwohl eine Zeile geloescht war
    (260923-dua)."""
    js = _js(baue_html([], ["e1"]))
    assert "if (alle) geloescht.forEach" not in js
    assert "geloescht.forEach" in js


def test_ruecknahme_trifft_keine_wieder_angelegte_position():
    """Die Loeschliste wird nie geleert. Legt man dieselbe Position neu an,
    loeschte die alte Ruecknahme sie sofort wieder."""
    js = _js(baue_html([], ["e1"]))
    assert "const gesendet = new Set();" in js
    assert "gesendet.has(" in js
