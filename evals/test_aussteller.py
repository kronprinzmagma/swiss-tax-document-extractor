...
def test_make_key_different_belegtyp_different_key() -> None:
    ...
    k1 = make_key("KK-H", "kk_praemienbescheinigung")
    k2 = make_key("KK-H", "spenden")
    assert k1 != k2
...
def test_fingerprint_stable() -> None:
    ...
    fp1 = make_fingerprint("BANK-U", "bank_zinsausweis", 2024)
    fp2 = make_fingerprint("BANK-U", "bank_zinsausweis", 2024)
    assert fp1 == fp2
    assert len(fp1) == 12
...
def test_fingerprint_different_year() -> None:
    ...
    fp_2024 = make_fingerprint("BANK-U", "bank_zinsausweis", 2024)
    fp_2025 = make_fingerprint("BANK-U", "bank_zinsausweis", 2025)
...
def test_load_valid_roundtrip(tmp_path: Path) -> None:
    ...
    original: dict[str, AusstellerEntry] = {
        "ubs__bank_zinsausweis": {
            "aussteller_name": "BANK-U",
            "belegtyp": "bank_zinsausweis",
            "last_seen_year": 2024,
            "fingerprint": make_fingerprint("BANK-U", "bank_zinsausweis", 2024),
        }
    }
...
def test_update_new_entry() -> None:
    ...
    result = update(history, "BANK-U", "bank_zinsausweis", 2024)
    expected_key = make_key("BANK-U", "bank_zinsausweis")
    assert expected_key in result
    entry = result[expected_key]
    assert entry["aussteller_name"] == "BANK-U"
    assert entry["belegtyp"] == "bank_zinsausweis"
    assert entry["last_seen_year"] == 2024
    assert entry["fingerprint"] == make_fingerprint("BANK-U", "bank_zinsausweis", 2024)


def test_update_idempotent_year_max_ascending() -> None:
    """Zweimal updaten mit 2023 dann 2024 → ``last_seen_year=2024``."""
    history: dict[str, AusstellerEntry] = {}
    update(history, "BANK-U", "bank_zinsausweis", 2023)
    update(history, "BANK-U", "bank_zinsausweis", 2024)
    key = make_key("BANK-U", "bank_zinsausweis")
...
def test_find_missing_excludes_seen() -> None:
    ...
    history: dict[str, AusstellerEntry] = {}
    update(history, "BANK-U", "bank_zinsausweis", 2023)
    seen = {make_key("BANK-U", "bank_zinsausweis")}
...
def test_find_missing_includes_last_year() -> None:
    ...
    history: dict[str, AusstellerEntry] = {}
    update(history, "KK-H", "kk_praemienbescheinigung", 2023)
    missing = find_missing(history, seen_keys=set(), current_year=2024)
    assert len(missing) == 1
    assert missing[0]["aussteller_name"] == "KK-H"
...
def test_find_missing_sorted_output() -> None:
    ...
    history: dict[str, AusstellerEntry] = {}
    update(history, "BANK-Z", "bank_zinsausweis", 2023)
    update(history, "BANK-U", "bank_zinsausweis", 2023)
    update(history, "KK-H", "kk_praemienbescheinigung", 2023)
    missing = find_missing(history, seen_keys=set(), current_year=2024)
    # bank_zinsausweis < kk_praemienbescheinigung; innerhalb bank: BANK-U < BANK-Z
    assert [m["aussteller_name"] for m in missing] == ["BANK-U", "BANK-Z", "KK-H"]
...
def test_write_atomic_creates_bak(tmp_path: Path) -> None:
    ...
    first: dict[str, AusstellerEntry] = {
        "ubs__bank_zinsausweis": {
            ...
            "belegtyp": "bank_zinsausweis",
            "last_seen_year": 2023,
            "fingerprint": make_fingerprint("BANK-U", "bank_zinsausweis", 2023),
        }
    }
...
def test_write_sort_keys_deterministic(tmp_path: Path) -> None:
    ...
    history: dict[str, AusstellerEntry] = {
        "zkb__bank_zinsausweis": {
            "aussteller_name": "BANK-Z",
            "belegtyp": "bank_zinsausweis",
            "last_seen_year": 2024,
            "fingerprint": make_fingerprint("BANK-Z", "bank_zinsausweis", 2024),
        },
        "helsana__kk_praemienbescheinigung": {
            "aussteller_name": "KK-H",
            "belegtyp": "kk_praemienbescheinigung",
            "last_seen_year": 2024,
            "fingerprint": make_fingerprint("KK-H", "kk_praemienbescheinigung", 2024),
        },
    }
    write_json(path, history)
    text = path.read_text(encoding="utf-8")
    # Keys alphabetisch sortiert — unabhaengig von den konkreten Namen
    erster, zweiter = sorted(history)
    assert text.index(erster) < text.index(zweiter)
