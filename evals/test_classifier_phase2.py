...
def test_classify_bank_zinsausweis_words():
    assert classify(_words("Steuerausweis", "Zinsausweis", "ZKB")) == (
        "bank_zinsausweis",
        None,
    )
...
def test_classify_kk_words():
    assert classify(
        _words("Prämienbescheinigung", "Krankenkasse", "Helsana")
    ) == ("kk_praemienbescheinigung", None)


# (d) Säule 3a.
def test_classify_saeule_3a_words():
    assert classify(_words("Säule", "3a", "Bescheinigung", "VIAC")) == (
        "saeule_3a",
        None,
    )
