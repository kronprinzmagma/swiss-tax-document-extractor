...
2. Lauf 2 mit nur einem Lohnausweis (ohne Bank-Beleg) erkennt den Vorjahres-
   Aussteller ZKB als fehlend und schreibt ihn in die Report-Sektion
...
def test_aussteller_lifecycle_end_to_end(tmp_path: Path, model_check: dict) -> None:
    ...
    # Lauf 1: nur Bank-Zinsausweis (ZKB / Hans Muster / 2024).
    shutil.copy(
        FIXTURES / "bank_zinsausweis_standard.pdf",
        input_run1 / "bank_zinsausweis_standard.pdf",
    )
    # Lauf 2: nur Lohnausweis — ZKB fehlt → muss als „Fehlend" auftauchen.
    shutil.copy(
    ...
    # aussteller.json existiert, hat den ZKB-Eintrag (D-B3 Key-Pattern).
    ...
    # --- Lauf 2: nur Lohnausweis, ZKB-Bank fehlt → find_missing-Treffer ---
    ...
    # Report-Sektion „Fehlende Vorjahres-Aussteller" enthält ZKB + bank_zinsausweis.
    ...
    assert "ZKB" in report_text, f"Report enthält kein 'ZKB':\n{report_text}"
