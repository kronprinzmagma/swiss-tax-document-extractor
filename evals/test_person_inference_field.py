...
* KK-KK-H-Pattern: ``versicherte_person_name=Lina Muster`` (Beitrags-
...
def _bank(name: str, tag_id: int = 11) -> BankZinsausweisRaw:
    ...
    return BankZinsausweisRaw(
        institut=TaggedField[str](value="BANK-Z", tag_refs=[1]),
...
def _kk(name: str, tag_id: int = 21) -> KkPraemienbescheinigungRaw:
    return KkPraemienbescheinigungRaw(
        kasse=TaggedField[str](value="KK-H", tag_refs=[1]),
...
def _saeule3a(name: str) -> Saeule3aRaw:
    return Saeule3aRaw(
        stiftung=TaggedField[str](value="STIFTUNG-V", tag_refs=[1]),
...
def _wertschriften(name: str) -> WertschriftenverzeichnisRaw:
    return WertschriftenverzeichnisRaw(
        institut=TaggedField[str](value="BANK-U", tag_refs=[1]),
...
def test_field_match_kk_lina_only(family: Family) -> None:
    """KK-KK-H-Pattern: versicherte_person_name=Lina Muster → kind1.

    Löst die Phase-2-KK-H-xfail: dort steckten ``Hans Muster``
