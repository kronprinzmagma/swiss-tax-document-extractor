...
def test_drei_gruppierungen_vorhanden():
    """Nach Beleg, nach Belegart, nach Ziffer — je nach Arbeitsschritt."""
    doc = baue_html(_zeilen(), ["elternteil_1"])
    for modus in ("nach Beleg", "nach Belegart", "nach Ziffer"):
        assert modus in doc, modus
    assert "function renderNachTyp(" in doc
...
def test_eigene_zeilen_werden_vor_dem_zustand_eingehaengt():
    ...
    Neuladen weg."""
    doc = baue_html(_zeilen(), ["elternteil_1"])
    einhaengen = doc.index("neue.forEach(z => { if (!R.some(x => x.nr === z.nr)) R.push(z); });")
...
def test_suche_greift_in_allen_ansichten():
    ...
    assert "!istGestrichen(r) && passtZurSuche(r)" in doc  # Übertragen
...
def test_speicher_wird_ergaenzt_statt_neu_gebaut():
    ...
    # Der alte, zerstoererische Aufbau darf nicht zurueckkommen.
    assert "const raus = {};" not in doc
