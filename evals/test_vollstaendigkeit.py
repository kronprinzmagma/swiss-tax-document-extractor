...
def z(person, beschreibung, pdf="a.pdf",
      belegtyp="kk_praemienbescheinigung") -> dict:
...
def test_selbstkosten_ohne_praemie_ist_unstimmig():
    """Der vom Nutzer genannte Fall — geprueft je Person."""
    ...
    assert b[0].betrifft == "elternteil_1"
...
def test_praemie_in_anderem_dokument_reicht():
    """Gibt es Praemien, reicht es — egal in welchem Dokument.

    Frueher galt die Regel je Dokument. Eine Leistungsabrechnung weist selbst
    getragene Kosten aus und nie eine Praemie; die steht auf der
    Praemienbescheinigung. Die Regel meldete damit einen Befund, den niemand
    schliessen konnte (260923-dua).
    """
    zeilen = [
        z("elternteil_1", "Prämie Grundversicherung KVG", "praemien.pdf"),
        z("elternteil_1", "Selbst getragene Krankheits- und Unfallkosten",
          "abrechnung.pdf", belegtyp="krankheitskosten"),
    ]
    assert pruefe_dokument_konsistenz(zeilen) == []


def test_auch_die_zusatzversicherung_zaehlt_als_praemie():
    zeilen = [
        z("elternteil_1", "Prämie Zusatzversicherung VVG", "praemien.pdf"),
        z("elternteil_1", "Selbst getragene Krankheits- und Unfallkosten",
          "abrechnung.pdf", belegtyp="krankheitskosten"),
    ]
    assert pruefe_dokument_konsistenz(zeilen) == []
...
def test_die_praemie_einer_anderen_person_hilft_nicht():
    """Die Frage gilt je Person, nicht fuer die Familie als Ganzes."""
    zeilen = [
        z("elternteil_2", "Prämie Grundversicherung KVG", "praemien.pdf"),
        z("elternteil_1", "Selbst getragene Krankheits- und Unfallkosten",
          "abrechnung.pdf", belegtyp="krankheitskosten"),
    ]
    b = pruefe_dokument_konsistenz(zeilen)
